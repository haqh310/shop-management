...
public class GlobalExceptionHandler {
    ...
    // public Object handle404(NoResourceFoundException ex, HttpServletRequest
    // request) {

    // // Kiểm tra xem request này có phải từ các đường dẫn hệ thống ngầm, API hoặc
    // // công cụ dev không
    // String uri = request.getRequestURI();
    // if (uri.startsWith("/.well-known") || uri.startsWith("/api") ||
    // uri.endsWith(".json")) {
    ...
    // System.err.println("Hệ thống gặp lỗi logic tại: " + request.getRequestURI() +
    // " -> " + ex.getMessage());

    // // Tương tự, nếu lỗi xảy ra ở API thì trả về JSON thay vì trả về giao diện
    // HTML
    // if (request.getRequestURI().startsWith("/api")) {
    // return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    // .body(Map.of("error", "Internal Server Error", "message", "Đã xảy ra lỗi hệ
    // thống"));
    // }
